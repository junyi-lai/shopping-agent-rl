...
from shopping_agent.collection.sft import (
    acceptance_reasons,
...
class SftCollectionTests(unittest.TestCase):
    ...
    def test_build_artifacts_excludes_held_out_tasks_and_sanitizes_accepted_rows(self):
        ...
        # accepted.jsonl 已经是可训练行:只保留 messages/tools/task_id/trajectory_id,
        # 审计字段(Teacher 私有推理、Reward 文本、steps/terminal_result)不进训练数据。
        self.assertEqual({row["task_id"] for row in accepted}, {1, 2})
        for row in accepted:
            self.assertEqual(
                sorted(row), ["messages", "task_id", "tools", "trajectory_id"]
            )
        self.assertEqual(accepted[0]["messages"][-1]["content"], "购买已完成。")
    ...
    def test_quality_score_prefers_more_evidence_and_fewer_repeats(self):
        from shopping_agent.collection.sft import _trajectory_quality_score

        good = {
            "steps": [
                {"tool_name": "search_products", "parameters": {"query": "a"}},
                {"tool_name": "open_product", "parameters": {"asin": "1"}},
        ...
        self.assertGreater(_trajectory_quality_score(good), _trajectory_quality_score(bad))
