...
class DivergentGenerationPromptTokenizer(CharacterTokenizer):
    """模拟部分 chat template 中 generation prompt 与实际 assistant 起始略有差异。"""
...
class SftTrainingTest(unittest.TestCase):
    ...
    def test_openai_tool_call_json_string_is_normalized_for_chat_template(self):
        """chat template 要求参数是 mapping，原始 OpenAI messages 的 JSON 字符串不能直接传入。"""
        messages = [
            {
                "role": "assistant",
                "content": "",
