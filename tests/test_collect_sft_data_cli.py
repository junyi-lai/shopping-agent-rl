...
from shopping_agent.collection.collect import (
    _validate_args,
    batch_paths,
...
class CollectSftDataCliTests(unittest.TestCase):
    def test_defaults_match_the_current_teacher_collection_contract(self):
        ...
        self.assertEqual(args.output_dir, Path("works/collect"))
    ...
    def test_batch_paths_keep_raw_and_derived_files_together(self):
        ...
        self.assertEqual(paths["rejected"], Path("outputs/example/rejected.jsonl"))
        self.assertEqual(paths["metadata"], Path("outputs/example/metadata.json"))
    ...
    def test_collection_stops_at_the_accepted_target(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, patch(
            "shopping_agent.collection.collect.collect_for_task", side_effect=rows
        ) as collect, patch(
            "shopping_agent.collection.collect.acceptance_reasons", return_value=(True, [])
        ):
    ...
    def test_accepted_target_ignores_held_out_rows_already_in_raw(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "shopping_agent.collection.collect.collect_for_task",
                return_value={"trajectory_id": "two", "task_id": 2},
            ) as collect, patch(
                "shopping_agent.collection.collect.acceptance_reasons",
                return_value=(True, []),
            ):
                written, accepted = collect_until_target(
                    tasks=[{"task_id": 2}],
    ...
    def test_parallel_collection_uses_an_independent_client_per_trajectory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, patch(
            "shopping_agent.collection.collect.collect_for_task", side_effect=collect
        ), patch(
            "shopping_agent.collection.collect.acceptance_reasons", return_value=(True, [])
        ):
