...
手写任何模型的特殊 token 或 tool-call 格式。
"""
...
def normalize_messages_for_chat_template(messages):
    """将 OpenAI 风格 tool-call 参数转成目标 chat template 可渲染的 mapping。

    原始 JSONL 保持 OpenAI 标准：``function.arguments`` 是 JSON 字符串。部分模板
    （如工具调用）会遍历 arguments 的键值对，因此只在训练前复制并转换；无法解析
    为 object 的调用应被上层作为不可训练样本丢弃。
    """
