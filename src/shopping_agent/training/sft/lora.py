...
from shopping_agent.evaluation.blind_guard import guard_blind_final
from shopping_agent.training.sft.dataset import (
...
def parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument("--swanlab-project", default="shopping-agent", help="SwanLab project 名")
...
def _load_preprocessing_components(model_name, auto_tokenizer, revision=None):
    """加载 tokenizer，并让它同时承担 chat template 的职责。

    目标模型是纯文本 CausalLM，数据只含文本与工具调用，因此 chat template 由
    tokenizer 提供，labels 用 tokenizer 的 token id。
    """
...
def main(argv: list[str] | None = None):
    ...
    if args.curriculum_manifest and not args.validation:
        raise SystemExit("课程训练必须提供 --validation（通常与 --train 指向同一 SFT 数据文件）")
