...
from shopping_agent.reward.versions import active_version_string
...
def validate_reward(raw_detail: object) -> dict:
    """Validate and minimize public Environment v2.1 terminal reward diagnostics."""
    if not isinstance(raw_detail, Mapping):
        raise ValueError("reward_detail must be an object")
    if raw_detail.get("reward_version") != active_version_string():
        raise ValueError("reward_detail has an unsupported reward_version")
    reward_type = str(raw_detail.get("reward_type", ""))
    if reward_type not in REWARD_VERSION_TYPES:
    ...
    return {
        "reward_version": active_version_string(),
...
def reward_breakdown(state: dict) -> dict[str, float | bool]:
    ...
    if state.get("reward_version") == active_version_string():
        ...
        # Reward v4: terminal Reward v4 + process shaping, shared with evaluation.
        shaped = compute_shaped_reward(
            terminal_utility,
            distinct_searches=count_distinct_searches(state["steps"]),
            distinct_opened_candidates=count_distinct_opened_candidates(state["steps"]),
            purchase_success=purchase_success,
            steps=len(state.get("steps") or []),
            evidence_actions=count_evidence_actions(state.get("steps") or []),
            repeat_action_count=count_consecutive_duplicate_actions(
                state.get("steps") or []
            ),
            reward_type=state.get("reward_type"),
            reward_valid=reward_valid,
            max_steps=int(state.get("max_steps", 35)),
        )
        ...
        return {
            ...
            "native": native,
            "semantic": semantic,
            "efficiency": shaping["efficiency_bonus"],
            "penalty_unfinished": 0.0,
            "penalty_repeat": shaping["repeat_penalty"],
            "repeat_action_rate": (
                int(state.get("repeat_action_count", 0))
                / max(int(state.get("action_attempt_count", 0)), 1)
            ),
            "shaped_reward_version": active_version_string(),
            "shaping": shaping,
            "total": shaped["total"],
            "terminal_utility": terminal_utility,
