...
from shopping_agent.environment.actions import action_reject_reason
from shopping_agent.environment.tools import tool_call_to_action
...
class ShopSimulatorTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs):
        ...
        if step["done"]:
            ...
            if result.get("over") is not True:
                _mark_infrastructure_invalid(state, "invalid_terminal_result")
            else:
                # 环境只返回原始事实;由项目奖励包计算终局分类(Reward v4 结构)。
                try:
                    raw_detail = compute_terminal_reward_detail(result)
                    public_detail = validate_reward(raw_detail)
                except Exception as exc:
                    _mark_infrastructure_invalid(
                        state,
                        f"invalid_terminal_reward_detail:{exc.__class__.__name__}:{exc}",
                    )
                else:
                    ...
                    state["final_reward"] = float(public_detail["terminal_utility"])
                    step["reward"] = state["final_reward"]
