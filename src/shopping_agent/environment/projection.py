...
from shopping_agent.environment.actions import clickable_buttons, product_ids
from shopping_agent.environment.product_id import PRODUCT_ID_CAPTURE, is_product_id
...
def _project_structured_search_results(
    ...
    *,
    ...
    if len(product_lines) > int(search_top_k):
        raise ObservationProjectionError(
            f"raw search page has {len(product_lines)} products, above configured "
            f"page capacity {int(search_top_k)}"
        )
    raw_buttons = clickable_buttons(observation)
