"""Turn raw Teacher rollouts into the accepted, trainable SFT rows.
...
Reward v4 gold purchases, keeps one trajectory per task, excludes held-out tasks
and strips audit-only Teacher content. ``accepted.jsonl`` therefore already has
the row shape the filter stage promotes into ``data/sft/all.jsonl``.
"""
...
from shopping_agent.environment.actions import (
    RUNTIME_GUARD_FIELD,
    action_reject_reason,
)
from shopping_agent.environment.tools import SHOP_TOOL_SCHEMAS, tool_call_to_action
...
def _trajectory_quality_score(trajectory: dict) -> float:
    """Reward v4 质量分。所有已验收轨迹终局均为 gold,故终局分固定取 1.0。"""
    steps = trajectory.get("steps") or []
...
def build_collection_artifacts(
    *,
    ...
    quality_rejected = 0
    accepted_by_task: dict[int, list[dict]] = {}
    ...
    for trajectory in read_jsonl(raw_path):
        ...
        accepted_by_task.setdefault(task_id, []).append(trajectory)

    # 同一题有多个"买对"轨迹时,按 V4 质量分取最高的一条,其余记作 duplicate_task。
    for task_id in sorted(accepted_by_task):
        candidates = sorted(
            accepted_by_task[task_id],
            key=_trajectory_quality_score,
            reverse=True,
        )
        accepted_rows.append(build_sft_row(candidates[0]))
        for extra in candidates[1:]:
            duplicate_tasks_excluded += 1
            reject_reasons.update(["duplicate_task"])
            rejected_rows.append(
                {
                    "trajectory_id": extra.get("trajectory_id"),
                    "task_id": task_id,
                    "status": extra.get("status"),
                    "reject_reasons": ["duplicate_task"],
                }
            )
    ...
    write_jsonl(paths["accepted"], accepted_rows)
