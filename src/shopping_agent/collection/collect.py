...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if not args.build_only and not args.llm_base_url:
        raise SystemExit(
            "missing teacher endpoint: set OPENAI_BASE_URL (or pass --llm-base-url). "
            "See .env.example; the teacher must be an OpenAI-compatible /chat/completions API."
        )
    if not args.build_only and not args.api_key:
        raise SystemExit(
            "missing teacher API key: set OPENAI_API_KEY (or pass --api-key). "
