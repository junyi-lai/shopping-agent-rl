...
from shopping_agent.environment.actions import action_guard_tool_message, action_reject_reason
from shopping_agent.environment.context import (
    ContextBudgetError,
    VllmChatTokenCounter,
    VllmTextTokenCounter,
    compact_chat_messages,
)
from shopping_agent.environment.projection import project_observation
from shopping_agent.environment.client import ShopAgentEnv, ShopEnvironmentError, ShopHttpError
from shopping_agent.environment.tools import (
    SHOP_TOOL_SCHEMAS,
    tool_call_to_action,
)
from shopping_agent.environment.observation import render_structured_observation
...
def _is_deepseek_model(model) -> bool:
    """True for DeepSeek-hosted model ids (``deepseek-flash``, ``deepseek-v4-pro`` ...).

    Used only to decide whether to send the provider-specific ``thinking`` field;
    the student model served by local vLLM has its own name and never matches.
    """

    return "deepseek" in str(model).casefold()
...
class OpenAIChatClient:
    ...
    def copy(self):
        """Return an independent client with its own token counters and no shared state.

        ``complete`` writes per-request fields (``last_context_tokens`` / ``last_context_event``),
        so concurrent evaluation tasks must not share one client across threads.
        """
        return OpenAIChatClient(**self._init_args)
    ...
    def complete(self, messages, tools):
        ...
        if self.thinking:
            ...
            # DeepSeek 的托管 API 默认开启 thinking,且**省略该字段不等于关闭**,
            # 必须显式传 disabled。否则模型把计划写进 reasoning_content、正文留空、
            # 不发 tool_call,采集会被判成 assistant_final 而整条被拒。
            # 该字段是服务商特有的,因此只按模型名发给 DeepSeek,绝不发给本地 vLLM
            # (学生模型的服务名来自 configs/pipeline.yaml,不会命中)。
...
def collect_tasks(
    ...
    if workers == 1:
        written = []
        for task, attempt_index in pending:
    ...
    # 并发路径:每个任务独占一个环境租约与一个独立 client(避免共享 last_context_* 竞态),
    # 结果仍按原始任务顺序写回,基础设施故障照旧中止。
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {}
        for task, attempt_index in pending:
            task_client = copy_client() if copy_client is not None else client
            future = pool.submit(
                collect_for_task,
                task,
                client=task_client,
            ...
            futures[future] = (int(task["task_id"]), attempt_index)
    ...
    written = []
    for task, attempt_index in pending:
        trajectory = results[(int(task["task_id"]), attempt_index)]
        append_jsonl(output_path, [trajectory])
        written.append(trajectory)
        if _is_infrastructure_failure(trajectory):
            raise CollectionInfrastructureError(
...
def _execute_tool_call(env, tool_call, step_index):
    ...
    # 环境只返回原始事实;由项目奖励包计算终局分类并写回 result,供汇总使用。
    if result.get("done") is True:
        try:
            detail = compute_terminal_reward_detail(result)
            result = dict(result)
            result["reward_detail"] = detail
            result["reward"] = float(detail["terminal_utility"])
            result["reward_valid"] = bool(detail["reward_valid"])
        except Exception:
            pass  # 无效终局保持原样;汇总层会按基础设施无效处理
    step.update(
        {
            "observation": observation,
