...
def build_data(run_dir):
    ...
    compact_summary = {
        ...
        "mean_reward": summary["mean_final_reward"],
                "metric_note": "主指标=严格成功率;平均 Reward(过程参考) 为过程参考指标,不作模型比较依据",
...
<main class="page">
  ...
  <section class="grid two">
    <div class="card"><h2>最终结果构成</h2><div id="outcome-chart"></div><div class="small">严格成功以 Reward v4 的 gold_purchase 计；未完成表示模型停止时没有终局 Reward。</div></div>
