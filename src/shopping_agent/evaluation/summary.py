...
def summarize_trajectories(expected_task_ids, trajectories):
    ...
    gold_purchases = sum(
        detail.get("reward_version") == REWARD_VERSION
    ...
    shaped_rewards = [
        _shaped_reward(item, detail)
        for item, detail in zip(by_task.values(), reward_details)
    ]
    final_rewards = [shaped["total"] for shaped in shaped_rewards]
    terminal_utilities = [
    ...
    shaping_totals = {
        key: sum(float(shaped["shaping"].get(key, 0.0)) for shaped in shaped_rewards)
        for key in (
            "efficiency_bonus",
            "evidence_bonus",
            "exploration_bonus",
    ...
    return {
        ...
        "primary_metric": "strict_success_rate",
        "shaped_reward_version": REWARD_VERSION,
        ...
        "reward_type_rates": {
            ...
            for reward_type in REWARD_VERSION_TYPES
        },
        ...
        "shaping": {
            key: value / len(shaped_rewards) if shaped_rewards else 0.0
            for key, value in shaping_totals.items()
        },
...
def _shaped_reward(item, detail):
    """Score one trajectory with the environment-independent reward package."""
    return score_reward_detail(detail, steps=item.get("steps") or [])
...
def _is_strict_success(trajectory):
    ...
    return (
        ...
        and is_strict_success(
            detail.get("reward_type"),
            detail.get("reward_valid"),
            detail.get("purchase_success"),
        )
    )
